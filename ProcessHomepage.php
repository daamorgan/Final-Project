    ...
    if ($_SERVER['REQUEST_METHOD']==='GET'){
      ...
      if (!(empty($filterOption)) &&  isset($filterOption)) {
        ...
        // 	echo $user;
          ...
          //result("SELECT I.id, I.title, I.type, I.status, U.firstname ,U.lastname, I.created FROM Issue I Join Users U ON I.assigned_to=U.id AND assigned_to='$id'" ,$conn);
        }else{
      ...
      <tr>
        ...
        <tr>
          <td>
            ...
            $id=$row["id"];
            $title=$row["title"];
            echo "<p>#$id <nav> <a class=issueTitle href=JobDetails.php id=$id>$title</span></a><nav>"; ?></td>
          <td><?= $row["type"];?></td>
          <td>
